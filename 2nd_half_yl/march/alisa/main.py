...
def handle_dialog(req, res):
    ...
    if req['session']['new']:
        ...
        res['response']['text'] = f'Привет! Купи {check}а!'
        # Получим подсказки
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        res['response']['text'] = f'{check}а можно найти на Яндекс.Маркете!'
        if check == 'слон':
            check = 'кролик'
        else:
            check = 'слон'
    ...
    res['response']['text'] = \
        f"Все говорят '{req['request']['original_utterance']}', а ты купи {check}а!"
    res['response']['buttons'] = get_suggests(user_id)
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2:
        suggests.append({
            "title": "Ладно",
            "url": f"https://market.yandex.ru/search?text={check}",
            "hide": True
        })
